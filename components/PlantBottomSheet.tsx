...
import { Sun, Droplet, Compass, ShoppingBag, X, Tag } from "lucide-react";
...
export function PlantBottomSheet({ plant, onClose, onAddToCart }: PlantBottomSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-stone-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      {/* Modal Container */}
      <div
        className="relative z-10 w-full max-w-4xl bg-white dark:bg-stone-900 rounded-3xl shadow-2xl border border-stone-100 dark:border-stone-800 overflow-hidden flex flex-col md:flex-row transform transition-all max-h-[90vh] text-stone-900 dark:text-stone-100"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <button
          onClick={onClose}
          type="button"
          className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-stone-900/40 hover:bg-stone-900/80 text-white flex items-center justify-center backdrop-blur-md transition-all shadow-md min-h-[44px] min-w-[44px]"
              ...
              <>
                ...
                <div className="flex items-center border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-800 rounded-2xl p-1 shrink-0">
                  <button
                    ...
                    disabled={quantity <= 1}
                    className="w-9 h-9 rounded-xl flex items-center justify-center text-stone-600 dark:text-stone-300 font-bold text-base disabled:opacity-40 hover:bg-stone-200 dark:hover:bg-stone-700 transition-colors"
                  >
                    -
                  </button>
                  ...
                  <button
                    ...
                    className="w-9 h-9 rounded-xl flex items-center justify-center text-stone-600 dark:text-stone-300 font-bold text-base hover:bg-stone-200 dark:hover:bg-stone-700 transition-colors"
                  >
                    +
                  </button>
                </div>
                ...
                <button
                  ...
                  className="flex-grow bg-terracotta hover:bg-[#b04a25] text-white font-semibold py-3.5 px-6 rounded-2xl shadow-md transition-all flex items-center justify-center gap-3 min-h-[48px]"
                >
